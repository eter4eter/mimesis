...
class RussiaSpecProvider(BaseSpecProvider):
    ...
    def snils(self) -> str:
        ...
        snils = [self.random.randint(0, 9) for _ in range(9)]

        def get_snils_sum(snils):
            first_part = snils[0] * 100 + snils[1] * 10 + snils[2]
            second_part = snils[3] * 100 + snils[4] * 10 + snils[5]
            third_part = snils[6] * 100 + snils[7] * 10 + snils[8]

            if first_part <= 1 and second_part <= 1 and third_part <= 998:
                return None

            s = 0
            for index, mult in enumerate(range(9, 0, -1)):
                s += snils[index] * mult
        ...
        snils = reduce(lambda x, y: x + y, map(lambda x: str(x), snils)) + get_snils_sum(snils)
        return '{}-{}-{}-{}'.format(snils[:3], snils[3:6], snils[6:9], snils[9:])
